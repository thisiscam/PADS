...
class RegularLanguage:
  ...
  def __hash__(self) -> int:
    return hash(self.recognizer.minimize())
...
class DFA(FiniteAutomaton):
    ...
    while unprocessed:
      ...
      for c in self.alphabet:
        ...
        if xc not in equivalences:
          ...
          return False
    for x, y in equivalences.items():
      if self.isfinal(x) != other.isfinal(y):
        return False
    return True
  ...
  def __hash__(self) -> int:
    ...
    The state is first topoligically sorted. Then the hash is computed by 
    hashing the edges and `isfinal` marks.
    """
    alphabet = sorted(self.alphabet)
    states = {
        x: i for i, x in enumerate(
            self._dfs_topological_traverse(self.initial, alphabet, set()))
    }
    is_finals = []
    edges = []
    for state, state_idx in states.items():
      is_finals.append(self.isfinal(state))
      for c in self.alphabet:
        next_state = self.transition(state, c)
        edges.append((state_idx, c, states[next_state]))
    return hash((tuple(is_finals), tuple(edges)))

  def _dfs_topological_traverse(self, state, alphabet, visited):
    if state in visited:
      return
    visited.add(state)
    yield state
    for c in alphabet:
      next_state = self.transition(state, c)
      yield from self._dfs_topological_traverse(next_state, alphabet, visited)
